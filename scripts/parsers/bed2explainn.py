...
@click.option(
    "-f", "--fraction",
    help="Build an atlas by mergin intervals.",
    type=float,
    default=.5,
    show_default=True
)
...
def main(**args):
    ...
    _to_ExplaiNN(args["bed_file"], args["genome_file"], args["dummy_dir"],
        args["output_dir"], args["prefix"], args["random_seed"], args["splits"])

def _to_ExplaiNN(bed_files, genome_file, dummy_dir="/tmp/", output_dir="./",
                 prefix=None, random_seed=1714, splits=[80, 10, 10]):
    ...
    # Ys
    enc = OneHotEncoder()
    arr = np.array(list(range(len(bed_files)))).reshape(-1, 1)
    enc.fit(arr)
    ...
    for i, bed_file in enumerate(bed_files):
        ...
        handle = open(a.seqfn)
        for interval, record in zip(a.intervals, SeqIO.parse(handle, "fasta")):
            y = ys[i]
            s = str(record.seq).upper()
            try:
                y[i] = float(interval.score)
            except:
                # i.e. not a float
                pass
    ...
    if df.shape[1] == 3 and df[2].nunique() == 1: # i.e. only one binary class
        data = []
